...
class PCDfull(Dataset):
    ...
    def __getitem__(self, index):
        ...
        print(np.max(img_t0_r))
        print(np.min(img_t0_r))
        img_t0 = np.asarray(img_t0_r).astype('f').transpose(2, 0, 1) / 255.0  # -- > (..., height, width)
        print(np.max(img_t0))
        print(np.min(img_t0))
        exit()
        img_t1 = np.asarray(img_t1_r).astype('f').transpose(2, 0, 1) / 255.0  # -- > (..., height, width)
        mask_ = np.asarray(mask_r>128).astype('f').transpose(2, 0, 1)               # -- > (..., height, width)
...
class PCDcrop(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_t0_r_ = np.asarray(img_t0_r).astype('f').transpose(2, 0, 1) / 255.0     # -- > (RGB, height, width)
        img_t1_r_ = np.asarray(img_t1_r).astype('f').transpose(2, 0, 1) / 255.0     # -- > (RGB, height, width)
        mask_r_ = np.asarray(mask_r>128).astype('f').transpose(2, 0, 1)             # -- > (RGB, height, width)
